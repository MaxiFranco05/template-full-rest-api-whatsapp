...
Define conversation flows using YAML configuration files
"""
import yaml
...
class YAMLFlowLoader:
    ...
    def load_all_flows(self):
        ...
        for flow_file in self.flows_directory.glob("*.yaml"):
            ...
            try:
                flows[flow_name] = self.load_flow_from_yaml(flow_name)
...
def create_example_flow_file():
    """Create an example flow YAML file"""
    flows_dir = Path("app/flows")
    flows_dir.mkdir(exist_ok=True)
    
    example_file = flows_dir / "welcome.yaml"
    with open(example_file, 'w', encoding='utf-8') as file:
        file.write(EXAMPLE_FLOW_YAML)
    
    print(f"Example flow file created: {example_file}")


# Factory function
def create_yaml_flow_loader(flows_directory: str = "app/flows") -> YAMLFlowLoader:
    """Create a YAML flow loader"""
    return YAMLFlowLoader(flows_directory)
